...
static DWORD WINAPI ThreadFunc (INJDATA *pData)
{
	...
	LV_ITEM _macro_lvi; 
		...
		_macro_lvi.cchTextMax = MAX_PATH; 
		_macro_lvi.pszText = (LPSTR)pData->pbText; 

		nXferred = pData->fnSendMessage(pData->hwnd, LVM_GETITEMTEXT, (WPARAM)(1), (LPARAM)(LV_ITEM *)&_macro_lvi);
...
{	
	...
	__try {
		...
		if (fUnicode)  wcscpy_s((LPWSTR) pbString, MAX_PATH,(LPCWSTR) DataLocal.pbText);
		else		   strcpy_s((LPSTR)pbString, MAX_PATH, (LPCSTR)DataLocal.pbText);
	}
	__finally {
		if ( pDataRemote != 0 )
